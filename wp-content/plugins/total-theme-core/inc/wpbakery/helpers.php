...
function vcex_enqueue_icon_font( $family = '', $icon = '' ) {
	...
	if ( 'fontawesome' == $family && false !== strpos( $icon, 'ticon' ) ) {
		return;
	}
...
function vcex_get_icon_class( $atts, $icon_location ) {
	...
	if ( 'fontawesome' == $icon_type && ! empty( $atts[ $icon_location ] ) ) {
		...
		if ( function_exists( 'wpex_convert_fa_to_ticon' ) && apply_filters( 'wpex_convert_fa_to_ticon', true ) ) {
			$ticon = wpex_convert_fa_to_ticon( $icon );

...
function wpex_recommended_plugins() {
	return apply_filters( 'wpex_recommended_plugins', array(
		'total-theme-core'       => array(
			...
			'source'             => 'https://totalwptheme.s3.us-east-1.amazonaws.com/plugins/total-theme-core/version-1-0-8/total-theme-core.zip',
			'required'           => true,
...
function wpex_get_default_content_area_layout() {
	...
	$mod = wpex_get_mod( 'content_layout' );
...
function wpex_parse_inline_lightbox_gallery( $attachments = array() ) {
	...
	foreach ( $attachments as $id ) {
		...
		if ( $gallery_image || $video ) {
			$count ++;
			$gallery[$count]['src'] = $video ? $video : $gallery_image;
			if ( $video && $gallery_image ) {
				$gallery[$count]['thumb'] = $gallery_image;
			}
			if ( $has_titles ) {
				$title = wpex_get_attachment_data( $id, 'alt' );
				if ( $title ) {
					$gallery[$count]['title'] = $title;
				}
			}
			if ( $has_captions ) {
